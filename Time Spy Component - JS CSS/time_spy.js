document.addEventListener("DOMContentLoaded",function(){
  ...
  // var timeSpies = document.querySelector(".evo_js-time-spy");
  // for(var i = 0; i < timeSpies.length; i++) {
  ...
  function updatePosition(){
    ...
    // If top of the content is close to the top of the page AND
    // 
    if ((content.offsetTop < windowMiddleAddScroll) && window.pageYOffset < window.innerHeight) {
      ...
      //  the top of timespy should be ( current scroll distance down - height of browser display window ) plus half of the height scroll
       ...
       console.log((window.pageYOffset - window.innerHeight)*.45);
      timeSpy.style.top = (((window.pageYOffset - window.innerHeight) * .45) + halfScrollHeight) + windowMiddleMinusScroll - parentRect.top + "px";
    ...
    if(((fullPage - fullParent) < window.innerHeight) && (window.pageYOffset > fullPage - (window.innerHeight * 2))) {
      ...
      timeSpy.style.top = window.pageYOffset + windowMiddleMinusScroll - content.offsetTop + ((-(fullPage - window.pageYOffset - (window.innerHeight * 2)))*.40) + "px";
      console.log("4");
    }
